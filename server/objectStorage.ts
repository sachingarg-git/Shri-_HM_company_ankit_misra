...
export class ObjectStorageService {
  ...
  // Gets the upload URL for a client document with predictable path.
  async getClientDocumentUploadURL(clientId: string, documentType: string): Promise<string> {
    const privateObjectDir = this.getPrivateObjectDir();
    if (!privateObjectDir) {
      throw new Error(
        "PRIVATE_OBJECT_DIR not set. Create a bucket in 'Object Storage' " +
    ...
    const fullPath = `${privateObjectDir}/uploads/${clientId}/${documentType}`;
